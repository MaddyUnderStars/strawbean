...
const testReminder: Macro<[MockApi.Message, Date, boolean]> = async (
	t: macroContext,
	message,
	expected,
	repeating = false,
	...
	if (!reply.embeds)
		return t.fail(`message did not contain embed : ${reply}`);
		...
		for (var i = 1; i <= 12; i++) {
			for (
				...
				const inputString = expected
					...
					.join("");
				const msg = new MockApi.Message(
					`remindme test at ${inputString} in ${i} ${unit}`,
				);
				await testReminder(
					t,
					msg,
					new Date(expected.valueOf() + i * units[unit]),
					false,
				);
			}
		}
		...
		for (var i = 1; i <= 12; i++) {
			for (
				...
				const inputString = expected
					...
					.join("");
				const msg = new MockApi.Message(
					`remindme test in ${i} ${unit} at ${inputString} `,
				);
				await testReminder(
					t,
					msg,
...
test("remindme test at [time] [date]", async (t) => {
	...
	const msg = new MockApi.Message(
		`remindme test at ${timeString} ${dateString}`,
	);
	await testReminder(t, msg, expected, false);
});
...
test("remove all", async (t) => {
	...
	for (var i = 0; i < 10; i++) {
		await awaitReply(
			t.context.bot,
			new MockApi.Message("remindme test in 1 year", user),
		);
	}
...
test("rename latest example reminder", async (t) => {
	...
	for (var i = 0; i < 10; i++) {
		...
		await awaitReply(
			t.context.bot,
			new MockApi.Message("rename latest example reminder", user),
		);
		await wait(100);
	}
...
test("rename 1 example reminder", async (t) => {
	...
	await awaitReply(
		t.context.bot,
		new MockApi.Message("rename 1 example reminder", user),
	);
	const reminders = await t.context.bot.Env.libs.reminders.getAll(user.id);
...
test("time latest in 1 week", async (t) => {
	...
	await awaitReply(
		t.context.bot,
		new MockApi.Message("remindme test in 5 years", user),
	);
