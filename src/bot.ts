...
export default class Bot {
	...
	disconnectHandler = (e: Error | Discord.CloseEvent) =>
	...
	messageCreate = async (msg: Discord.Message): Promise<any> => {
		...
		if (msg.guild)
			guild = (await this.getCache(msg.guild.id, "guilds", {
				_id: msg.guild.id,
		...
		if (
			msg.content.indexOf(pingString) === 0 &&
			msg.content.trim().length === pingString.length
		)
			return msg.reply(
				`Hey, for a list of commands and their usage, use \`help\`.\n\n` +
					`Your personal prefix is \`${user.prefix}\`.\n` +
					(guild
						? `This guilds prefix is \`${guild.prefix}\`\n`
						: "") +
		...
		if (
			content.indexOf(pingString) === 0 &&
			content.trim().length > pingString.length
		)
			argsStartPos = pingString.length + 1;
		//was the ping used instead of prefix?
		...
		else if (!msg.guild) argsStartPos = 0; //are we in dm?
		else return; //no valid prefix found

		//kinda bad solution but whatever, it'll work.
		var replies: Array<{
			returnValue: Types.CommandReturnValue;
			command: string;
		}> = [];

		var commands = content
			.substring(argsStartPos)
		...
		for (var curr of commands) {
			...
			if (replies.length === 5 && commands.length > 6) {
				replies.push({
					returnValue: {
						reply: "You cannot chain more than 5 commands.",
					},
					command: "curr",
				});
				break;
			...
			var alias =
				user.alias[args[0].toLowerCase()] ??
				this.Env.defaultAliases[args[0].toLowerCase()];
			...
			if (!toRun) return;
			if (
				"commandChainingLimit" in toRun &&
				replies.filter((x) => x.command === cmd).length >
					toRun.commandChainingLimit
			)
				continue;
			...
			try {
				...
				if (!ret) continue;
				if (ret?.reply)
					replies.push({ command: cmd, returnValue: ret });
			} catch (e) {
				await msg.react("💢");
				console.error(e);
				await this.dmOwner(
					`Error thrown by message: \`${msg.content}\` by user \`${msg.author.id}\` ( \`${msg.author.username}\` )\`\`\`${e}\`\`\``,
				);
		...
		if (replies.length < 2) {
			if (replies[0]?.returnValue?.reply)
				await msg.reply(replies[0].returnValue.reply);
			return;
		}
		...
		await msg.reply({
			embeds: replies.map(
				(x) =>
					(x.returnValue.reply as Discord.ReplyMessageOptions)
						.embeds[0],
			),
		});
	};
	...
	private dmOwner = async (
		msg: string | Discord.MessagePayload,
		...
		try {
			...
			console.error(
				`I couldn't send a dm to my owner (${
					process.env.OWNER
				}). I'm probably offline;\nDm contents : ${JSON.stringify(
					msg,
				)}`,
			);
		}
