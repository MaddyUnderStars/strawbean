...
export default new (class desc implements Types.Command {
	...
	exec = async ({ user, args, Libs }: Types.CommandContext) => {
		...
		if (!desc || !list[id])
			return {
				reply: "Sorry, you must provide a valid reminder ID, followed by the description. Eg: `rename 1 you should vibe really hard!`",
			};

		await Libs.reminders.setDescription(
			user._id,
			list[id]._id,
			desc.join(" "),
		);
		...
		return {
			reply: {
				embeds: [
					{
						...ret.embeds[0],
						title: "Reminder description set",
