...
export default new (class time implements Types.Command {
	name = "time";
	usage =
		'{id || "latest"} ["at" [date || weekday] [time]] [{"in" || "every"} {time}]';
	help =
		"Changes the time of a reminder. Same effect as deleting a reminder, and recreating it with a new time";
	examples = [
	...
	exec = async ({ user, args, Libs }: Types.CommandContext) => {
		...
		if (!rawTimeString || !list[id])
			return {
				reply: "Sorry, you must provide a valid reminder ID, followed by the description. Eg: `time 1 in 1 second!`",
			};
