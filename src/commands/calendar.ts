...
export default new (class calendar implements Types.Command {
	...
	help =
		"Generates/displays your calendar URL for importing into Google Calendar or other services.";
	...
	exec = async (context: Types.CommandContext) => {
		...
		if (!args.length || args[0] !== "generate") {
			return {
				reply: "To generate a calendar URL, rerun this command with `generate`.",
			};
		}
		...
		if (args[0] === "generate") {
			...
			collection.updateOne(
				{ _id: user._id },
				{ $set: { calendarToken: user.calendarToken } },
			);
