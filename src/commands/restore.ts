...
export default new (class restore implements Types.Command {
	...
	exec = async ({ user, args, Libs }: Types.CommandContext) => {
		...
		deleted = Object.values(deleted).sort(
		...
		if (!args.length) {
			if (deleted.length === 0)
				return { reply: "You have no reminders in cache" };

			var items = deleted.map(
				(x, i) =>
					`#${i + 1} : ${new Date(x.time).toLocaleString(
						user.locale,
						{
			...
			return {
				reply:
					"Showing recently sent/deleted reminders. " +
		...
		if (!restored)
			return { reply: "Please provide a valid ID to restore." };
		...
		const created = await Libs.reminders.reinstate(
			restored.owner,
			restored._id,
			parsed.seconds,
			parsed.offset || Date.now(),
		);

		return { reply: Libs.reminders.prettyPrint(created) };
	};
})();
