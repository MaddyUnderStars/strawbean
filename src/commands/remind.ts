...
export default new (class remind implements Types.Command {
	name = "remind";
	usage =
		'{description} ["at" [date || weekday] [time]] [{"in" || "every"} {time}]';
	...
	exec = async ({ user, args, Libs, msg }: Types.CommandContext) => {
		...
		var parsed = Libs.language.parseString(
			args.join(" "),
			user.locale,
			user.timezone,
		);
		...
		if (!parsed) {
			const format =
				dateFormats[user.locale] ||
				dateFormats[process.env.DEFAULT_LOCALE];
			return {
				reply: `We couldn't parse that, sorry. Make sure you're following the format \`${format} HH:MM (AM/PM)\``,
			};
		}
		...
		if (!parsed.seconds && !parsed.offset) {
			if (user.defaultTime) {
				...
				var attempt = Libs.language.parseString(
					" " + user.defaultTime,
					user.locale,
					user.timezone,
				);
		...
		if (!parsed.seconds && !parsed.offset) {
			...
			return {
				reply: "Sorry, the time you entered couldn't be properly interpreted. Try using `remindme` without any arguments to use the extended flow.",
			};
		}
		...
		var id =
			(await Libs.reminders.getAll(msg.author.id)).filter(
				(x) => x._id.toString() === ret._id.toString(),
			)[0].remove_id + 1;
