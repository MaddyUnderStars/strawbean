...
export default new (class expand implements Types.Command {
	...
	usage = "{id || 'latest'}";
	help =
		"Lists all information about a specific reminder, such as the full description and link to the message that created it.";
	examples = ["expand latest", "expand 1"];
	exec = async ({ user, args, Libs }: Types.CommandContext) => {
		...
		if (!list[id])
			return {
				reply: "Sorry, you must provide a valid reminder ID. Check out the `list` command!",
			};
