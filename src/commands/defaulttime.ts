...
export default new (class defaulttime implements Types.Command {
	...
	help = "Set a default time for reminders, to be used when no time is provided.";
	examples = [
	...
	exec = async ({ user, args, Libs, Env }: Types.CommandContext) => {
		...
		const parsed = Libs.language.parseString(" " + time, user.locale, user.timezone);
		...
		const date = new Date((parsed.offset || Date.now()) + parsed.seconds).toLocaleString(
			user.locale,
			{
		...
		return {
			reply: `Your default time string has been set to \`${time}\`.\n` +
				`If you were to create a reminder using this, it would send at \`${date}\`` +
				(parsed.repeating ? `, repeating every \`${prettyMilliseconds(parsed.seconds, { verbose: true })}\`` : "")
		};
	};
});